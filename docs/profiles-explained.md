...
| `--width` / `--height` | 空Latentノードすべて（2つを同時に指定する）。プロファイルが解像度を持つノードを列挙している場合は、そのすべてへ**倍率を掛けて**書く |
...
`--width` と `--height` が指すのは**基準解像度**で、保存される画像の寸法とは限りません。
1つのグラフが解像度を複数箇所に述べることがあり、その関係はプロファイルごとに違います。

| プロファイル | 解像度を述べるノード | 倍率 |
|---|---|---|
| `wai-hires` | `EmptyLatentImage`（base pass）、`ImageScale`（Hires後） | 1、**1.5** |
| `wai-controlnet` | 上の2つ ＋ ラフ線画を揃える `ImageScale` | 1、**1.5**、1 |
| `flux2-klein-edit` | `EmptyFlux2LatentImage`、`Flux2Scheduler` | 1、1 |

`wai-hires` で `--width 832 --height 1216` を指定すると、base pass が 832×1216、保存される画像は 1248×1824 になります。
倍率はプロファイルに書きますが、ワークフローの現在値と突き合わせて検証されるので、片方だけ編集するとvalidateが落ちます。

**倍率を掛けた値も8の倍数である必要があります。**
`wai-hires` の 1.5倍のノードは `ImageScale` ですが、その出力は `VAEEncode` へ入るので8の倍数の制約を受けます。
実測したところ、`--width 840` は倍率先を 1260×1836 にし、ComfyUIはこれを 1256×1832 へ黙って切り詰めました。
記録には 1260×1836 が残るので、記録が出力寸法について嘘をつきます。
そのため8の倍数にならない指定は実行前にエラーにします。
この2プロファイルで使えるのは実質16の倍数です。

`flux2-klein-edit` は2箇所とも倍率1なので、両方が同じ値で同時に動きます。
